...
def get_token_from_keyring(env: str) -> str | None:
...
def get_token(env: str, config: PublishConfig) -> str | None:
...
def read_pyproject_toml(package_path: Path) -> dict[str, Any] | None:
...
def detect_version(package_path: Path) -> tuple[str | None, str]:
...
def read_version_from_file(file_path: Path) -> str | None:
...
def resolve_attribute_version(package_path: Path, attr_path: str) -> str | None:
...
def upload_package(
    ...
    if artifacts:
        ...
        package_name = match.group(1) if match else name.split("-")[0]
